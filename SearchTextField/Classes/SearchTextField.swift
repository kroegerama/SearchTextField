    ...
    fileprivate func filter(forceShowAll addAll: Bool) {
        ...
        for i in 0 ..< filterDataSource.count {
            ...
            if !inlineMode {
                ...
                let titleFilterRange = (item.title as NSString).range(of: query, options: comparisonOptions)
                let subtitleFilterRange = item.subtitle != nil ? (item.subtitle! as NSString).range(of: query, options: comparisonOptions) : NSMakeRange(NSNotFound, 0)
